...
proc main =
  const ImagesDirPath = "testdata/image/remove_plugin"
  ...
  block:
    ...
    check not image.removePlugin(plugin).err.isSome
  ...
  block:
    ...
    check res.err.isSome
    check err.kind == pluginDoesNotExistInImageError
