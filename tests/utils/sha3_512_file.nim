...
proc main =
  const testdataDirPath = "testdata/utils/sha3_512File"

  block:
    const testdataPath = testdataDirPath / "dir"
    let err = sha3_512File(testdataPath).err.get()

    check err.kind == fileDoesNotExistError
    check err.filePath == testdataPath

  block:
    check sha3_512File(testdataDirPath / "foo.txt").res ==
    ...
    check sha3_512File(testdataDirPath / "bar/baz.md").res ==
    ...
    check sha3_512File(testdataDirPath / "archive.zip").res ==
